...
public class SoftKeyboard extends InputMethodService 
    ...
    private LatinKeyboard mKannadaKeyboard;
    ...
    @Override public void onInitializeInterface() {
        ...
        mKannadaKeyboard = new LatinKeyboard(this, R.xml.kannada);
    ...
    @Override public View onCreateInputView() {
        ...
        SharedPreferences theme=getSharedPreferences("app",Context.MODE_PRIVATE);
        String selected_theme=theme.getString("theme","blue");
        switch (selected_theme){
            ...
            case "dark":
                mInputView = (LatinKeyboardView) getLayoutInflater().inflate(
                        R.layout.input_dark, null);
                break;
            case "green":
                mInputView = (LatinKeyboardView) getLayoutInflater().inflate(
                        R.layout.input_green, null);
                break;
            case "orange":
                mInputView = (LatinKeyboardView) getLayoutInflater().inflate(
                        R.layout.input_orange, null);
                break;
            case "red":
                mInputView = (LatinKeyboardView) getLayoutInflater().inflate(
                        R.layout.input_red, null);
                break;
            case "cyan":
                mInputView = (LatinKeyboardView) getLayoutInflater().inflate(
                        R.layout.input_cyan, null);
                break;
            case "brown":
                mInputView = (LatinKeyboardView) getLayoutInflater().inflate(
                        R.layout.input_brown, null);
                break;
        }
        ...
        switch (attribute.inputType & InputType.TYPE_MASK_CLASS) {
            ...
            case InputType.TYPE_CLASS_TEXT:
                ...
                try{
                    ...
                    if(current.equals(mQwertyKeyboard) || current.equals(mSymbolsKeyboard)){
                        mCurKeyboard = mQwertyKeyboard;
                        mPredictionOn = true;
                        // We now look for a few special variations of text that will
                        // modify our behavior.
                        int variations = attribute.inputType & InputType.TYPE_MASK_VARIATION;
                        if (variations == InputType.TYPE_TEXT_VARIATION_PASSWORD ||
                                variations == InputType.TYPE_TEXT_VARIATION_VISIBLE_PASSWORD) {
                            // Do not display predictions / what the user is typing
                            // when they are entering a password.
                            mPredictionOn = false;
                        }

                        if (variations == InputType.TYPE_TEXT_VARIATION_EMAIL_ADDRESS
                                || variations == InputType.TYPE_TEXT_VARIATION_URI
                                || variations == InputType.TYPE_TEXT_VARIATION_FILTER) {
                            // Our predictions are not useful for e-mail addresses
                            // or URIs.
                            mPredictionOn = false;
                        }
    ...
    public void onKey(int primaryCode, int[] keyCodes) {
        ...
        if (isWordSeparator(primaryCode)) {
            ...
            return;
        }else if (primaryCode == -111  && mInputView != null) {
            Keyboard current = mInputView.getKeyboard();
            if (current == mQwertyKeyboard) {
                setLatinKeyboard(mKannadaKeyboard);
            }
            return;
        }else if (primaryCode == -112  && mInputView != null) {
            Keyboard current = mInputView.getKeyboard();
            if (current == mKannadaKeyboard) {
                setLatinKeyboard(mQwertyKeyboard);
            }
            return;
